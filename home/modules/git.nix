  ...
  programs.git = {
    ...
    settings = {
      ...
      alias = {
        ...
        land = "!f () { unset GIT_DIR GIT_WORK_TREE GIT_COMMON_DIR; b=$(git branch --show-current); if [ -z \"$b\" ]; then echo \"detached HEAD; nothing to land\" >&2; exit 1; fi; main_wt=$(git worktree list --porcelain | sed -n '1s/^worktree //p'); target=$(git -C \"$main_wt\" symbolic-ref --short -q HEAD); if [ \"$target\" != main ]; then echo \"main checkout $main_wt is on '$target', expected main\" >&2; exit 1; fi; if [ \"$b\" = main ]; then echo \"already on main; nothing to land\" >&2; exit 1; fi; git -C \"$main_wt\" merge --ff-only \"$b\" || exit 1; echo \"landed $b on main; next: git -C $main_wt push origin main\"; }; f";
      };
